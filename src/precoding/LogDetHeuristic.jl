...
function calculate_logdet_rates(state::LogDetHeuristicState, settings)
    K = length(state.W)
...
function calculate_MMSE_rates(state::LogDetHeuristicState, settings)
    K = length(state.W)
    ds = Int[ size(state.W[k], 1) for k = 1:K ]; max_d = maximum(ds)
...
function calculate_allocated_power(state::LogDetHeuristicState)
    K = length(state.W)
    ds = Int[ size(state.W[k], 1) for k = 1:K ]; max_d = maximum(ds)
